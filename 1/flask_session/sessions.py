...
class SaeKVDBSession(dict,SessionMixin):
    pass
...
class SaeKVDBInterface(SessionInterface):
    
    serializer = pickle
    session_class = SaeKVDBSession

    def __init__(self, kv, key_prefix):
        if kv is None:
            import sae.kvdb
    ...
    def open_session(self, app, request):
        ...
        val = self.kv.get(str(self.key_prefix + sid))
        ...
        if val is not None:
            try:
                ...
                return self.session_class(sid=sid)
        _session = self.session_class(sid=sid)
    ...
    def save_session(self, app, session, response):
        ...
        if not session:
            if session.modified:
                self.kv.delete(self.key_prefix + session.sid)
        ...
        #一定要设置app.permanent_session_lifetime，见index.wsgi
        self.kv.set(str(self.key_prefix + session.sid), val,
